...
function resetViewTimeout() {
    viewTimeout = true;

    // Set viewTimeout to false after 10 seconds
    stopCheetahButton.classList.add('inactive');
    const timeoutId = setTimeout(() => {
        viewTimeout = false;
        console.log("Timeout reached, viewTimeout set to false.");
        stopCheetahButton.classList.remove('inactive');
    }, 10000);

    // Set viewTimeout to false if the Enter key is pressed
    document.addEventListener("keydown", function handleSpaceKey(event) {
        if (event.key === "Shift") {
            viewTimeout = false;
            stopCheetahButton.classList.remove('inactive');
            console.log("Space key pressed, viewTimeout set to false.");
            clearTimeout(timeoutId); // Cancel the timeout
            document.removeEventListener("keydown", handleSpaceKey); // Clean up listener
        }
    });
}
        ...
        if (!isNaN(input) && input !== ' ') { // Check if input is a number
            if (currentInput.length < 3) {
                ...
                if (currentInput.length === 3) {
                    ...
                    // Look up the bidder
                    // This needs to be more robust. Find all entries starting with those three digits (huh?)
                    ...
                    if (fullName !== "No matching bidders found.") {
                        ...
                        renderHistory();
                        if (audioMode) {
                            // need to call a function here to timeout. A method of cancellling it would be nice as well
                            resetViewTimeout();
                        }
...
const audioModeReadout = document.getElementById('audio-activity');
const ACCESS_KEY = "fHjA85EdVHQ8iBMjJ9sjF6fl2UGTqxxmcbD//HyihWvYwTmaZpRplw==";
...
function trackWords(word) {
    const number_mapping = {
        'zero': '0', 'one': '1', 'too':'2', 'to': '2','two': '2', 'three': '3', 'four': '4','for': '4',
        'five': '5', 'six': '6', 'seven': '7', 'eight': '8', 'mine':'9','nine': '9'
    };

    // Split on whitespace or on hyphens.
    const lowerWords = word.toLowerCase().split(/[\s-]+/);

    for (const lowerWord of lowerWords) {
        // Translate the word to a digit if it's valid
        if (number_mapping[lowerWord]) {

            handleInput(number_mapping[lowerWord]);

        } else if(lowerWord) {
            console.log("heard: "  + lowerWord + ", clearing transcript");
            // clear transcript.

            handleInput('Enter');
            // need some persistence
            // when a word is spoken 
            // created viewTimeout in response
        }
    }
...
async function startCheetah() {
    if (!audioMode) {
        audioMode = true;
        audioModeReadout.innerHTML = "Loading…";
        if (!cheetah) {
            await initCheetah();
        }
    
        await WebVoiceProcessor.WebVoiceProcessor.subscribe(cheetah);
        audioModeReadout.innerHTML = "Listening…";
    } else {
        console.log("audio mode already on!")
    }
}
...
async function stopCheetah() {
    if (audioMode) {
        ...
        audioModeReadout.innerHTML = "Audio Mode Inactive";
    } else {
        console.log("Audio mode already stopped");
    }
}
